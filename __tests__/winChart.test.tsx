...
describe('WinChart', () => {
  ...
  it('renders a View with correct accessibility props', () => {
    ...
    expect(getByRole('image', { name: 'Win distribution pie chart' })).toBeTruthy();
  });
  ...
  it('calculates and formats labels correctly for full data', () => {
    ...
    expect(pieChartCall.data[0].label).toBe(`${((10 / totalWins) * 100).toFixed(0)}%`);
    expect(pieChartCall.data[1].label).toBe(`${((20 / totalWins) * 100).toFixed(0)}%`);
    expect(pieChartCall.data[2].label).toBe(`${((30 / totalWins) * 100).toFixed(0)}%`);
    expect(pieChartCall.data[3].label).toBe(`${((15 / totalWins) * 100).toFixed(0)}%`);
    expect(pieChartCall.data[4].label).toBe(`${((5 / totalWins) * 100).toFixed(0)}%`);
  });
