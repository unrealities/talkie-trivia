...
describe('ErrorBoundary', () => {
  ...
  it('displays the error UI when an error occurs', () => {
    ...
    expect(screen.getByText('An unexpected error occurred. Please try reloading the app.')).toBeTruthy();
  });
  ...
  it('componentDidCatch logs the error and errorInfo', () => {
    ...
    expect(console.error).toHaveBeenCalledWith('ErrorBoundary details:', error, errorInfo);
  });
  ...
  it('handleReload successfully reloads the app', async () => {
    ...
    expect(console.log).toHaveBeenCalledWith('Attempting to reload app via Updates API...');
  });
