...
describe("HintUI", () => {
  ...
  it("disables toggle button when isToggleDisabled is true", () => {
    const { getByText } = render(
      <HintUI {...defaultProps} isToggleDisabled={true} />
    )
  ...
  it("passes correct hintsAvailable to HintButton", async () => {
    render(
      <HintUI {...defaultProps} showHintOptions={true} hintsAvailable={2} />
    )
    const decadeButtonParent = screen.getAllByText("Decade")[0].parent
    const directorButtonParent = screen.getAllByText("Director")[0].parent
    const actorButtonParent = screen.getAllByText("Actor")[0].parent
    ...
    expect(decadeButtonParent?.props.accessibilityLabel).toContain(
      "2 hints available."
    )
    expect(directorButtonParent?.props.accessibilityLabel).toContain(
      "2 hints available."
    )
    expect(actorButtonParent?.props.accessibilityLabel).toContain(
      "2 hints available."
    )
    expect(genreButtonParent?.props.accessibilityLabel).toContain(
      "2 hints available."
    )
  }, 10000)
})
