...
describe("Facts Component", () => {
  ...
  it("renders error state", () => {
    const { getByText } = render(
      <Facts movie={mockMovie} error={"An error occurred"} />
    )
  ...
  it("shows alert when canOpenURL returns false", async () => {
    ...
    await waitFor(() => {
      // waitFor is needed because Alert.alert might take some time to be called
      expect(mockAlert).toHaveBeenCalledWith(
        "Unsupported Link",
  ...
  it("shows alert on link opening error", async () => {
    ...
    const mockOpenURL = jest
      .spyOn(Linking, "openURL")
      .mockRejectedValue(new Error("Failed to open link"))
    const mockCanOpenURL = jest
      .spyOn(Linking, "canOpenURL")
      .mockResolvedValue(true)
    const { getByLabelText } = render(<Facts movie={mockMovie} />)
