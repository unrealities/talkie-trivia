...
describe("GoogleLogin", () => {
  ...
  it('renders "Sign In with Google" button when not signed in', () => {
    const { getByText } = render(
      <GoogleLogin player={null} onAuthStateChange={mockOnAuthStateChange} />
    )
  ...
  it('renders "Sign Out {playerName}" button when signed in and currentUser.displayName is null', () => {
    ...
    const player: { id: string; name: string; email: string } = {
      id: "player123",
      name: playerName,
      email: "foo@bar.com",
    }
    ...
    const { getByText } = render(
      <GoogleLogin player={player} onAuthStateChange={mockOnAuthStateChange} />
    )
  ...
  it("calls onAuthStateChange when the auth state changes", async () => {
    ...
    const handleSignInMock = jest.fn(() =>
      mockOnAuthStateChange({ id: "1", name: "foo", email: "foo@bar.com" })
    )
    ;(useGoogleAuth as jest.Mock).mockReturnValue({
      isLoading: false,
      authError: null,
